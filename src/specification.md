...
2. 路由器在转发某条广播时，首先需要检查的就是，路由器是否向该模块申请过该路径上的广播，如果该模块发来了路由器不需要的广播，则该条消息会被丢弃，并且通知该模块以后不要再发了。在通知模块之后的一分钟内，如果路由器又收到了之前未注册path以及其父级的广播，则直接丢弃，不过当过了一分钟，则收到一条，"错误计数器"就 + 1。
...
4. 当路由器需要向模块申请广播，首先清除第2条中明确不接收广播path以及其的子级的记录，之后向模块发送"broadcast\_open"消息,超时为3分钟，如果连续重试3次模块都没有回应，则断开连接。
5. 自己不允许监听自己的广播